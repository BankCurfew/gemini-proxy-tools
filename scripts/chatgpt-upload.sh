#!/bin/bash
# chatgpt-upload.sh — Upload image/file to ChatGPT conversation
# Usage: ./chatgpt-upload.sh /path/to/image.png [--tab ID] [--chat "message after upload"]
# Attaches file to ChatGPT input, optionally sends a follow-up message.

set -euo pipefail

FILE="${1:?Usage: chatgpt-upload.sh /path/to/image.png [--tab ID] [--chat \"message\"]}"
shift
TAB_ID=""
CHAT_MSG=""
while [[ $# -gt 0 ]]; do
...
if [ "$UPLOAD_OK" = "ok" ]; then
  echo "[OK] File attached to ChatGPT input"
